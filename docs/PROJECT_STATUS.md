...
- 修得見込の科目は、メイン画面の時間割プレビューで前学期・後学期の曜日時限（月〜金、6・7限は授業がある場合のみ）へ並べられる（Ver.1.2.0〜）
  - ターム開講の科目は対応する学期に含め、期間が重ならないターム同士は重複扱いしない。通年注記の科目は両学期に出す
  - カードは必修を赤、それ以外を要件データの科目区分ごとの固定色（13色）で色分けし、凡例を出す
  - オンデマンド・集中講義・土曜などの授業は表の下の別枠。曜日時限の候補が複数ある科目・低学年／再履修の科目・学域特別講義のテーマはドロップダウンで選べ、時限自体が決まらない科目は理由付きで別枠に出す
...
- ビルド時に要件データのチャンクが大きいという警告が出る（動作には影響なし）。年度ごとの分割読み込みで初回表示を軽くできる
- WebMCP のツールに、時間割プレビューの内容を読む操作はまだ無い（必要になれば追加）
