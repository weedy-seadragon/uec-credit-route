...
## 次のUI改善タスク（ベータテスト由来・未着手）

ベータテストで「必要な行動が分かりにくい」「操作部が小さく、一覧の整列も弱い」という意見が出た。次の順で検討・実装する。

1. 「不可の単位」を、再履修が必要であることが分かる見出しと説明文にする。必修の不合格科目には「再履修して単位を修得する必要があります」と明示する。
2. 選択必修・選択科目の各区分に、「この一覧からあと何単位（目安として何科目）を選んで修得するか」を見出し直下に表示する。必修の再履修と、候補から必要単位を選ぶ区分を混同させない。
3. 科目行と操作部を、スマホでも押しやすい大きさにする。科目名・単位・理由または状態・曜日時限が縦横に揃うグリッドを設計する。
4. ドロップダウンをすべて置き換えない。履修状態・学期フィルタ・その他単位認定のように選択肢が少ない箇所だけラジオボタン等を検討し、プログラム選択や科目検索のように候補が多い箇所はドロップダウンを維持する。
