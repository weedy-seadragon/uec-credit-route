...
4. 今回スコープ外にした主な項目：F-2c（友人アプリ取り込み）・F-2d（成績表貼り付け）、同時限警告（上記1番の設計判断待ち）、F-5（科目一覧・詳細）、F-6（プログラム比較）、F-9のOGP/SEO/sitemap（SPEC上もフェーズ4の予定）、夜間主の国際科目（付録C C.4、3・4年次のみ・上級科目扱い）の組み込み、Ⅱ・Ⅲ類・夜間主の審査（2年次終了時審査等）データ自体の作成（reviews.tsの実装自体は完了しているので、データさえ用意すれば表示は自動的に有効になる）
