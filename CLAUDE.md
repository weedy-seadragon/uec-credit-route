...
5. **共通単位の実装（未着手）**：メイン画面に「共通単位」専用のセクションをまだ作っていない。今は各区分の見出し（例:「類共通基礎（選択）（10/8単位）」）でオーバーフローが見えるだけで、それが共通単位としてどう積み上がっているかは表示できていない。開発者からの指定イメージ：
   ```
   共通単位 (n/N単位)
   ▼あぶれ分（デフォルトが展開）
    ・あぶれた分の単位
   ・取得した単位
   ```
   - 「あぶれ分」＝他の区分の必要単位を超えて修得し、共通単位に繰り入れられた分（各区分のoverflow合計。学修要覧2.5.1のルール、CLAUDE.md本文にも記載）。他の折りたたみ（留学生のみ等）と違って**デフォルトで展開**しておく
   - 「取得した単位」＝オーバーフロー経由ではなく、そもそも共通単位としてしか数えない科目（例: 言語文化応用科目Ⅱ、第二外国語の応用科目＝`commonCreditSources.alwaysCommon`、理数基礎科目の「物理学演習第一・第二」「化学概論第二」のように`countAs: 'common'`指定のもの）
   - n（実際に共通単位として認定された分）は、8（media の commonCredits）のような上限でキャップされているはず。今のrequirements.tsのどこでこの値を計算・キャップしているか確認してから実装すること
6. **スマホ対応（未着手）**：現状 `src/index.css` はほぼ最小構成（フォント・marginくらい）で、メディアクエリやレイアウトのレスポンシブ対応が無い。スマホの狭い画面でメイン画面（表・折りたたみ・プルダウンが多い）が崩れずに使えるかまだ確認していない。着手する際はまずPlaywrightでスマホサイズのビューポート（例: 375×667）に変更してdev serverを実機代わりに確認し、崩れている箇所（横スクロールが出る、ボタンが小さすぎる等）を洗い出してから直す
