...
1. **複数セクション科目の曜日時限解決（実装完了、データは拡充中）**：
   ...
   - **残作業**：`class_assignment_filled.csv`の未記入分（1343行中568行）を埋めれば、より多くの科目で曜日時限が表示されるようになる。`class_schedule.csv`にA2以降のデータを追記すれば、`build_class_assignment.py`再実行時に自動で埋まる行も増える
