...
1. **複数セクション科目の曜日時限解決**：上記の「重要な制約」の通り、時間割PDF（A1〜A8）とofferingsの教員名を突き合わせて、クラスごとの曜日時限を解決するデータ作業が必要。同時限警告（F-4関連）を実装するならここが前提になる
   - **着手を見送り（開発者判断、2026-09-05）**：シラバス上の科目名にクラス識別子が付いていない科目（実験科目・一部語学科目など）は、時間割PDFの格子を1コマずつ目視で書き起こしてシラバスの担当教員名と突き合わせる必要があると判明。1年次分（A1・A2）だけでも200コマ以上を目視で拾う規模の作業になるため、開発者と相談の上、今回は見送り他の作業を優先することにした。次回やるなら、まず1年次分だけに絞るのが現実的
