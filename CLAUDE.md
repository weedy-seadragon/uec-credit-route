...
1. **`scripts/fetch_syllabus.py`の実行確認**：上記の通りPython自体は使えるようになったが、これはシラバスサイトへの実際のアクセス（1秒以上間隔を空けた大量リクエスト）を伴うため長時間かかる。実行する場合は`gen_data.py`のあとに実行すること（逆だとofferingsが消える）
...
4. 今回スコープ外にした主な項目：F-2c（友人アプリ取り込み）・F-2d（成績表貼り付け）、審査（2年次終了時審査等）の合否表示（**UIとしてはⅠ類含めてまるごと未実装**。`src/`のコードは`reviews`を一切参照していない。データ自体もⅠ類5プログラムのJSONにしか無く、Ⅱ・Ⅲ類・夜間主は元データも無い。2026-09-05に確認・CLAUDE.md訂正）、同時限警告（上記の設計判断待ち）、F-5（科目一覧・詳細）、F-6（プログラム比較）、F-9のOGP/SEO/sitemap（SPEC上もフェーズ4の予定）、夜間主の国際科目（付録C C.4、3・4年次のみ・上級科目扱い）の組み込み
