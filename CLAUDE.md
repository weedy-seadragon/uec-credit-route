...
2. **シラバス取得の対象拡大（続き）**：調査した結果、当初の認識を訂正する必要があった（2026-09-05）：
   ...
   - **夜間主（学籍番号末尾s/t）は0件**。夜間主向けのシラバス一覧ページ（`GakkiIchiran_31_0.html`とは別のURL）がどこにあるか未確認。`GakkiIchiran_31_1.html`・`31_2.html`という別ページの存在は確認した（`200`で取得できる）が、中身が夜間主科目かどうかは未確認のまま（次回の課題）
