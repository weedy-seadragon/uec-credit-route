  ...
  return (
    <main>
      ...
      <section className="release-notes-section">
        ...
        <div className="release-note-entry">
          <p className="release-note-update">
            <span>・アップデート(Ver.1.1.8→Ver.1.2.0)</span>
            <span className="release-note-date">アップデート日 2026/9/26</span>
          </p>
          <h3 className="release-note-category">新機能：時間割プレビュー</h3>
          <ul className="release-note-items">
            <li>「修得見込」の科目を前学期・後学期の週間時間割（月〜金）で確認できます。6・7限は授業がある場合だけ表示し、スマートフォンでも横スクロールせずに見られます</li>
            <li>必修は赤、それ以外は卒業要件の科目区分ごとの固定色で表示し、凡例を付けました</li>
            <li>春・夏・秋・冬タームの科目を対応する学期にターム名付きで表示します。同じコマでも開講期間が重ならない科目は重複とせず、期間が重なる科目が複数ある場合は「同時限に2科目」と表示します</li>
            <li>オンデマンド・集中講義・土曜などの授業は表の下に分けて表示し、通年の情報工学工房は前学期・後学期の両方に表示します</li>
            <li>曜日時限の候補が複数ある科目や、低学年・再履修の科目は、表の下の選択欄から受講する授業を選べます。再履修向けの枠が1つだけなら、最初からその枠を表示します</li>
            <li>輪講・卒業研究・情報工学工房など曜日時限が決まっていない科目は、その理由を添えて表示します。科目ごとの表示・非表示は学年学期つき・学年学期順で選べ、選んだ授業と表示設定はこのブラウザに保存されます（履修記録・卒業判定には影響しません）</li>
          </ul>
          <h3 className="release-note-category">表示の改善</h3>
          <ul className="release-note-items">
            <li>メイン画面の大見出しを個別に折りたためるようにしました。開閉状態は次回も引き継がれます</li>
            <li>情報領域演習第三のA・B・Cクラスやインターンシップ（類別）など、曜日時限がない科目も、プロフィールに合った担当教員のシラバスへ移動できるようにしました</li>
